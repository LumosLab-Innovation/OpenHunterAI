 ...
 *
 * Aliases and budgets are read from environment variables (see .env.example
 * and LLM_PROVIDER_SPEC.md §4 / §7). Business logic only ever references
 * an alias (e.g. `llm.standard.reasoning`) — never a provider or model id.
 */
...
export const USE_CASE_TO_ALIAS: Record<LLMUseCase, ModelAlias> = {
  ...
  openhack_hunter_summary: 'llm.light.summary',
  strix_reasoning: 'llm.standard.reasoning',
  finding_classification: 'llm.standard.reasoning',
  severity_confidence: 'llm.standard.reasoning',
